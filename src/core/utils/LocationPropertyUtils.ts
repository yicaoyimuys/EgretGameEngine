 ...
 */
class LocationPropertyUtils extends BaseClass {
    public constructor() {
        super();
    }
     ...
     * */
    public getPara(paraName: string, paraUrl?: string): string {
     ...
     * 不传递paraUrl参数默认获取当前url
     * */
    public setProperty(paraName: string, paraValue: string, paraUrl?: string): string {
        var url = paraUrl || location.href;
        var urlPara = "&" + url.split("?")[1];
        if (url.indexOf("?") == -1) {
     ...
     * 相信你不会这么用的 =.=
     * */
    public hasProperty(paraName: string, paraUrl?: string): boolean {
        var url = paraUrl || location.href;
